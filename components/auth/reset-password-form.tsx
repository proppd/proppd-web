...
export function ResetPasswordForm({ authenticated }: Props) {
    ...
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm font-semibold text-amber-700">
          Password reset links can only be used once and expire after a short time. They also need to be opened in the same browser you requested them from. Request a fresh one from the sign-in screen.
        </p>
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (updateError) {
      const message = /session|jwt|token|missing|expired/i.test(updateError.message)
