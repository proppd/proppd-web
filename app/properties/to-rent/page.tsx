    ...
    <main className="min-h-screen bg-[#F5F7FA] text-[#050A30]">
      ...
      <section className="border-b border-slate-200 bg-white px-4 py-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <form action="/properties/to-rent" className="grid gap-3 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm xl:grid-cols-[1.6fr_1fr_1fr_auto] xl:items-end">
            ...
            <div className="flex flex-wrap gap-2 xl:justify-end">
              ...
              <a className="inline-flex min-h-11 items-center justify-center rounded-full border border-slate-300 bg-white px-4 text-sm font-black text-[#050A30] shadow-sm transition hover:border-[#3B49FF]" href="/properties/to-rent">
                Clear
              </a>
            </div>
          </form>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-sm font-black text-[#050A30]">
            ...
            {(filters.query || filters.propertyType || filters.bedrooms || filters.sort !== 'featured') ? (
              <a className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-slate-500 transition hover:border-[#3B49FF] hover:text-[#3B49FF]" href="/properties/to-rent">
                Clear all
              </a>
            ) : null}
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-4 rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-[-.05em] sm:text-4xl">{headline}</h1>
              <p className="mt-2 text-sm font-semibold text-slate-600">{filters.query ? 'Filtered across listing facts, areas, agents, and agencies.' : 'Rental cards with clear monthly pricing, verified routing, and POPIA-friendly enquiries.'}</p>
            </div>
            <a className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#050A30] px-5 py-3 text-sm font-black text-white" href={buildSavedSearchMailto(filters, { path: '/properties/to-rent', resultCount: rentListings.length })} aria-label="Request a saved rental search alert">
      ...
      <section className="px-4 pb-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
            <div>
              <p className="text-sm font-black uppercase tracking-[.2em] text-[#3B49FF]">Rental support</p>
              <h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Need a narrower rental brief?</h2>
            ...
            <div className="rounded-[1.5rem] bg-[#F5F7FA] p-4">
              <div className="flex items-center gap-2 text-sm font-black text-[#050A30]"><ShieldCheck size={16} className="text-[#3B49FF]" /> Verified routing and POPIA-friendly enquiry handling</div>
              <p className="mt-2 text-sm leading-6 text-slate-600">Start with the current filters, then refine by suburb, price band, and property type.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-5 lg:grid-cols-3">
            <section className="rounded-[2rem] bg-white p-6 shadow-sm">
              <p className="text-sm font-black uppercase tracking-[.2em] text-[#3B49FF]">Top rental areas</p>
              <h2 className="mt-2 text-2xl font-black tracking-[-.04em]">{areaWatchlist.length} active rental pockets</h2>
              <p className="mt-2 text-sm leading-6 text-slate-600">Current results are concentrated in a few rental-friendly suburbs, so we surface the strongest pockets first.</p>
              <div className="mt-5 grid gap-3">
                {areaWatchlist.length > 0 ? areaWatchlist.map((area) => (
                  <a key={area.label} href={buildToRentHref(params, { q: area.label })} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-[#F5F7FA] px-4 py-3 text-sm font-black text-[#050A30] transition hover:border-[#3B49FF] hover:text-[#3B49FF]">
            ...
            <section className="rounded-[2rem] bg-[#050A30] p-6 text-white shadow-sm lg:col-span-1">
              ...
              <h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Refine without losing the good rentals.</h2>
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-[#eefcf9] p-6 shadow-sm">
              ...
              <h2 className="mt-2 text-2xl font-black tracking-[-.04em] text-[#0f766e]">Turn the current rental set into a clean handoff.</h2>
              <p className="mt-4 text-sm font-bold leading-6 text-[#0f766e]">Save the search, then email it to a tenant or co-buyer with the filters already captured.</p>
              <div className="mt-5 flex flex-wrap gap-3">
                <a className="rounded-full bg-[#050A30] px-5 py-3 text-sm font-black text-white" href={buildSavedSearchMailto(filters, { path: '/properties/to-rent', resultCount: rentListings.length })}>Save search email</a>
