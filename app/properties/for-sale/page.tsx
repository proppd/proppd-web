    ...
    <main className="proppd-page">
      ...
      <section className="border-b border-[#E5E7EB] bg-white px-4 py-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <form action="/properties/for-sale" className="rounded-xl border border-[#E5E7EB] bg-white p-3 shadow-sm sm:p-4">
            <label className="flex min-h-12 items-center gap-3 rounded-full border border-slate-300 bg-white px-4 text-sm font-bold text-[#9CA3AF] focus-within:border-[#4A3AFF] focus-within:ring-4 focus-within:ring-[#4A3AFF]/10 sm:min-h-14 sm:px-5">
              <Search size={21} className="shrink-0 text-[#4A3AFF]" />
              <input
                name="q"
                type="search"
            ...
            <div className="mt-3 flex justify-end gap-2">
              <a className="inline-flex min-h-11 items-center justify-center rounded-full border border-slate-300 bg-white px-5 text-sm font-bold text-[#1A1A2E] shadow-sm transition hover:border-[#4A3AFF]" href="/properties/for-sale">
                Clear
              </a>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-sm font-bold text-[#1A1A2E]">
            {filters.query ? <FilterChip href={buildForSaleHref(params, { q: null })}>Search: “{filters.query}” <X size={14} /></FilterChip> : null}
            {filters.propertyType ? <FilterChip href={buildForSaleHref(params, { propertyType: null })}>Type: {filters.propertyType} <X size={14} /></FilterChip> : null}
            {filters.minPrice !== undefined ? <FilterChip href={buildForSaleHref(params, { minPrice: null })}>Min: R{filters.minPrice.toLocaleString('en-ZA')} <X size={14} /></FilterChip> : null}
            {filters.maxPrice !== undefined ? <FilterChip href={buildForSaleHref(params, { maxPrice: null })}>Max: R{filters.maxPrice.toLocaleString('en-ZA')} <X size={14} /></FilterChip> : null}
            {filters.bedrooms ? <FilterChip href={buildForSaleHref(params, { bedrooms: null })}>Beds: {filters.bedrooms}+ <X size={14} /></FilterChip> : null}
            {filters.bathrooms ? <FilterChip href={buildForSaleHref(params, { bathrooms: null })}>Baths: {filters.bathrooms}+ <X size={14} /></FilterChip> : null}
            {filters.parking ? <FilterChip href={buildForSaleHref(params, { parking: null })}>Parking: {filters.parking}+ <X size={14} /></FilterChip> : null}
            {filters.sort !== 'featured' ? <FilterChip href={buildForSaleHref(params, { sort: 'featured' })}>Sort: {filters.sort.replace('-', ' ')} <X size={14} /></FilterChip> : null}
            {(filters.query || filters.propertyType || filters.minPrice !== undefined || filters.maxPrice !== undefined || filters.bedrooms || filters.bathrooms || filters.parking || filters.sort !== 'featured') ? (
              <a className="inline-flex items-center gap-1 rounded-full border border-[#E5E7EB] bg-white px-3 py-2 text-[#9CA3AF] transition hover:border-[#4A3AFF] hover:text-[#4A3AFF]" href="/properties/for-sale">
                Clear all
              </a>
            ) : null}
          </div>
      ...
      <section className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-4 rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-sm sm:flex-row sm:items-center">
            <div>
              <p className="text-sm font-bold uppercase tracking-[.2em] text-[#4A3AFF]">For sale</p>
              <h1 className="mt-2 text-3xl font-bold tracking-[-.05em] sm:text-4xl">{headline}</h1>
              <p className="mt-2 text-sm font-semibold text-[#6B7280]">{filters.query ? 'Filtered across listing facts, areas, agents, and agencies.' : 'Price-first cards, verified status, and quick facts for buyers.'}</p>
            </div>
            <a className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#4A3AFF] px-5 py-3 text-sm font-bold text-white" href={buildSavedSearchMailto(filters, { path: '/properties/for-sale', resultCount: saleListings.length })} aria-label="Request a saved for-sale search alert">
          ...
          <div className="mt-6">
            <SortSelect
              value={filters.sort}
              options={sortOptions.map(({ value, label }) => ({ value, label, href: buildForSaleHref(params, { sort: value }) }))}
            />
          </div>
      ...
      <section className="px-4 pb-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
          <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
            <div>
              <p className="text-sm font-bold uppercase tracking-[.2em] text-[#4A3AFF]">Buyer support</p>
              <h2 className="mt-2 text-2xl font-bold tracking-[-.04em]">Need something more specific?</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#6B7280]">
                Save a search request and send the details straight to Proppd so we can keep matching you with verified for-sale listings.
              </p>
            </div>
            <div className="rounded-lg bg-[#F7F8FA] p-4">
              <div className="flex items-center gap-2 text-sm font-bold text-[#1A1A2E]"><MapPin size={16} className="text-[#4A3AFF]" /> Build a saved search around your target area</div>
              <p className="mt-2 text-sm leading-6 text-[#6B7280]">Use the current filters as a starting point, then refine by suburb, price band, and property type.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-5 lg:grid-cols-3">
            <section className="rounded-xl bg-white p-6 shadow-sm">
              <p className="text-sm font-bold uppercase tracking-[.2em] text-[#4A3AFF]">Top sale areas</p>
              <h2 className="mt-2 text-2xl font-bold tracking-[-.04em]">{areaWatchlist.length} active sale pockets</h2>
              <p className="mt-2 text-sm leading-6 text-[#6B7280]">Current results are concentrated in a few buyer-friendly suburbs, so we surface the strongest pockets first.</p>
              <div className="mt-5 grid gap-3">
                {areaWatchlist.length > 0 ? areaWatchlist.map((area) => (
                  <a key={area.label} href={buildForSaleHref(params, { q: area.label })} className="flex items-center justify-between gap-3 rounded-2xl border border-[#E5E7EB] bg-[#F7F8FA] px-4 py-3 text-sm font-bold text-[#1A1A2E] transition hover:border-[#4A3AFF] hover:text-[#4A3AFF]">
            ...
            <section className="rounded-xl border border-[#E5E7EB] bg-[#EFF6FF] p-6 shadow-sm">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <a className="rounded-full bg-[#4A3AFF] px-5 py-3 text-sm font-bold text-white" href={buildSavedSearchMailto(filters, { path: '/properties/for-sale', resultCount: saleListings.length })}>Save search email</a>
...
function buildForSaleHref(params: Record<string, string | string[] | undefined>, updates: Record<string, string | number | null | undefined>) {
  ...
  return query ? `/properties/for-sale?${query}` : '/properties/for-sale';
}
