          ...
          <div className="overflow-hidden rounded-xl bg-[#1A1A2E] text-white shadow-sm">
            <div className="grid gap-8 p-8 sm:p-10 lg:grid-cols-[1fr_360px] lg:p-12">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {sourceOptions.map((option) => {
                      ...
                      return (
                        <a
                          key={option.value}
                          className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-[.16em] transition ${isActive ? 'border-white/40 bg-white text-[#1A1A2E]' : 'border-white/10 bg-white/8 text-white/75 hover:border-white/30 hover:text-white'}`}
              ...
              <div className="rounded-xl border border-white/10 bg-white/10 p-6 backdrop-blur">
                <p className="text-sm font-bold uppercase tracking-[.18em] text-white/60">Queue snapshot</p>
                ...
                <div className="mt-5 rounded-lg bg-white/10 p-4">
                  <p className="text-xs font-bold uppercase tracking-[.18em] text-white/50">Source mix</p>
                ...
                <div className="mt-5 rounded-lg bg-white/10 p-4">
                  <p className="text-xs font-bold uppercase tracking-[.18em] text-white/50">Top of queue</p>
          ...
          <section className="mt-8 grid gap-8 lg:grid-cols-[1fr_340px]">
            <div className="rounded-xl bg-white p-6 shadow-sm sm:p-8">
              <div className={`mb-6 rounded-[1.75rem] border px-5 py-4 text-sm font-bold ${moderationEnabled ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
                <span className="font-bold uppercase tracking-[.14em]">Lead moderation</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-bold uppercase tracking-[.2em] text-[#4A3AFF]">Lead queue</p>
                  <h2 className="mt-2 text-3xl font-bold tracking-[-.05em]">Newest enquiries</h2>
                </div>
              ...
              <form className="mt-6 grid gap-3 rounded-xl border border-[#E5E7EB] bg-[#F7F8FA] p-4 lg:grid-cols-[minmax(0,1fr)_180px_220px_220px_auto]" method="get">
                ...
                <div className="flex items-center gap-3 rounded-full border border-[#E5E7EB] bg-white px-4 py-3">
                  ...
                  <input
                    ...
                    name="q"
                    defaultValue={query}
                    className="min-w-0 flex-1 bg-transparent text-sm font-bold text-[#1A1A2E] outline-none placeholder:text-[#9CA3AF]"
                ...
                <select
                  ...
                  className="rounded-full border border-[#E5E7EB] bg-white px-4 py-3 text-sm font-bold text-[#1A1A2E] outline-none"
                ...
                <div className="flex gap-3">
                  ...
                  {hasFilters ? (
                    <a className="inline-flex items-center justify-center rounded-full border border-[#E5E7EB] bg-white px-5 py-3 text-sm font-bold text-[#6B7280] transition hover:border-[#4A3AFF] hover:text-[#4A3AFF]" href="/admin">
              ...
              {hasFilters ? (
                <div className="mt-4 flex flex-wrap gap-2 text-sm font-bold text-[#6B7280]">
              ...
              <div className="mt-6 overflow-hidden rounded-xl border border-[#E5E7EB]">
                <div className="hidden grid-cols-[1fr_120px_110px_100px_240px] gap-4 bg-[#F7F8FA] px-5 py-3 text-xs font-bold uppercase tracking-[.14em] text-[#9CA3AF] md:grid">
                  <span>Lead</span>
                  <span>Intent</span>
                  <span>Quality</span>
                  <span>Status</span>
                ...
                <div className="divide-y divide-slate-200">
                  {queue.length > 0 ? (
                    queue.map((lead) => (
                      <div key={lead.id} className="grid gap-4 px-5 py-5 transition hover:bg-[#F7F8FA] md:grid-cols-[1fr_120px_110px_100px_240px]">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <a className="font-bold text-[#1A1A2E] transition hover:text-[#4A3AFF]" href={`/admin/leads/${lead.id}`}>
                              {lead.name}
                            </a>
                            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[.12em] text-[#9CA3AF]">{lead.id}</span>
                          </div>
                          <p className="mt-1 text-sm font-bold text-[#9CA3AF]">{lead.listingTitle}</p>
                          <p className="mt-1 text-xs font-bold uppercase tracking-[.14em] text-[#4A3AFF]">{getLeadSourceLabel(lead.sourcePage)}</p>
                          {lead.latestEventType ? (
                            <p className="mt-1 text-xs font-bold uppercase tracking-[.14em] text-[#9CA3AF]">
                          ...
                          <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#6B7280]">{lead.message}</p>
                          <div className="mt-3 flex flex-wrap items-center gap-2">
                            <a className="inline-flex rounded-full bg-white px-3 py-2 text-xs font-bold text-[#4A3AFF] ring-1 ring-slate-200 transition hover:ring-[#4A3AFF]" href={`/property/${lead.listingSlug}`}>
                              Open listing
                            </a>
                            <a className="inline-flex rounded-full bg-slate-100 px-3 py-2 text-xs font-bold text-[#6B7280] transition hover:bg-slate-200" href={`mailto:${lead.email}?subject=Proppd follow-up for ${encodeURIComponent(lead.listingTitle)}`}>
                              Reply by email
                            </a>
                          </div>
                          {lead.flags.length > 0 && <p className="mt-2 text-xs font-bold text-red-600">Flags: {lead.flags.join(', ')}</p>}
                        </div>
                        <p className="text-sm font-bold text-[#4A3AFF]">{formatLeadIntent(lead.intent)}</p>
                        <p><span className={`rounded-full px-3 py-1 text-xs font-bold ${qualityStyles[lead.quality]}`}>{lead.quality}</span></p>
                        <p className="text-sm font-bold capitalize text-[#6B7280]">{lead.status}</p>
                    ...
                    <div className="px-5 py-10 text-center">
                      <p className="text-base font-bold text-[#1A1A2E]">No leads match the current filters.</p>
                      <p className="mt-2 text-sm font-bold text-[#9CA3AF]">Clear the search or switch the filters to bring the queue back.</p>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    moderationDigest.map((item) => (
                      <div key={item.name} className="rounded-2xl border border-white/10 bg-white/8 p-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="rounded-full bg-white/12 px-3 py-1 text-xs font-bold text-white/80">{item.action}</span>
                        </div>
                      </div>
                    ))
                  ) : (
...
function Metric({ label, value, tone = 'default' }: { label: string; value: number; tone?: 'default' | 'warning' }) {
  return (
    <div className="rounded-2xl bg-white p-4 text-[#1A1A2E]">
      <p className={`text-3xl font-bold ${tone === 'warning' ? 'text-red-600' : ''}`}>{value}</p>
      <p className="mt-1 text-xs font-bold uppercase tracking-[.14em] text-[#9CA3AF]">{label}</p>
    </div>
  );
}
...
function SourceChip({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/8 px-3 py-3">
      <p className="text-[11px] font-bold uppercase tracking-[.14em] text-white/50">{label}</p>
