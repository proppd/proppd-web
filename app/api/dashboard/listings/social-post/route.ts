...
import { isAiConfigured, parseListingFactsFromBody } from '@/lib/ai/listing-description';
...
export async function POST(request: NextRequest) {
  if (!isAiConfigured()) {
    return jsonError('AI social posts are not enabled on this deployment.', 503);
  }
  ...
  const result = await generateListingSocialPost(facts);
  if (!result.ok) {
    return jsonError(result.error, 422);
  }

  return NextResponse.json({ post: result.description });
}
